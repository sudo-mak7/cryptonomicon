...
export default {
  ...
  methods: {
    ...
    checkInvalidCoin() {
      ...
      // this.invalidCoin = !this.autocompleteResult
      if (this.autocompleteResult) { // дополнительная проверка на существование монеты при добавлении
    ...
    autocompleteCollectData() {
      const symbols = Object.keys(
          this.coinlistAPI.Data).map((e) => this.coinlistAPI.Data[e].Symbol
      )

      const fullNames = Object.keys(
          this.coinlistAPI.Data).map((e) => this.coinlistAPI.Data[e].FullName
      )
    ...
    resetDuplicateAndInvalidCoinError() {
      this.duplicate = false; this.invalidCoin = false
    },
    ...
    add() {
      ...
      if (!this.duplicate && !this.invalidCoin && !this.emptyName) {
        ...
        setInterval(async () => {
          const f = await fetch(`https://min-api.cryptocompare.com/data/price?fsym=${currentTicker.name}&tsyms=USD&api_key=27426a7b898a8560e6074bd08a4ed101682d88ecdab3f2b5f03789668e65ab53`)
